...
Записывать входные символы в стек до конца файла,
...
Если памяти не хватит, то об этом хотя бы можно узнать.
...
А системный стек, всего скорее, работает быстрее, чем ваш собственный.

Минусы подхода, — используется все та же память.
Символы входного файла буду храниться в переменных `c` на каждой итерации рекурсии.
Но системный стек может закончиться раньше чем память.
И мы об этом узнаем только благодаря падению программы.
На самом деле, есть иные способы перехватить это событие,
но нужно или писать на C++ или использовать системные сигналы.


#### C помощью fseek

Позиционироваться в конец входного файла и читать его с конца.
При каждом чтении менять позицию `fseek`, сдвигая ее к началу.
Сдвигать надо на 2 символа (1 прочитанный, 1 еще не прочитанный).
Такой способ будет работать на файла любого размера.
Но каждом `fseek` нужно позиционироваться по диску.
...
в стеке останется только результат вычислений.
