...
/**
 * @fn  Решает уравнение вида F(x) = 0 методом дихотомии.
 *      На вход подается функция F(X), и отрезок, 
 *      на котором происходят вычисления.
 */ 
...
/**
 * @fn  Решает уравнение вида x = f(x) методом итераций.
 *      На вход подается функция f(X), и отрезок, 
 *      на котором происходят вычисления.
 *      Перед использованием метода для каждой функции
 *      надо проверять условия применимости.
 */ 
 ...
 *      На вход подается функция F(X), ее производная и отрезок, 
 *      на котором происходят вычисления.
 *      Перед использованием метода для каждой функции
 *      надо проверять условия применимости.
 */ 
